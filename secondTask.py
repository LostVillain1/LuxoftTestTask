...
def createFigure(x_res, y_res, output_path):
    ...
    material.node_tree.nodes['Principled BSDF'].inputs['Metallic'].default_value = 100
    material.node_tree.nodes['Principled BSDF'].inputs['Base Color'].default_value = (0.113597, 0.113597, 0.8, 0.5)
    ...
    scene.render.filepath= f'{output_path}\\image2.png'
    bpy.ops.render.render(write_still=1)

    stop_time = datetime.datetime.now()
    #Creating JSON file
    output_dict = {'test_name': 'Test2',
    ...
    with open(f"{output_path}\\sample2.json", "w") as outfile:
